...
public class PropertyUtils {
	 ...
	 */
	public static Set<String> STOP_WORDS_FOR_SEARCH = new HashSet<String>();
	...
	static {
		...
		try (InputStream input = SSNoCAppInitializer.class.getClassLoader()
			...
			if (input != null) {
				...
				String stop_words_for_search = prop.getProperty("stop_words");
				if (stop_words_for_search != null) {
					String[] nameList = stop_words_for_search.split(" ");

					for (int i = 0; i < nameList.length; i++) {
						STOP_WORDS_FOR_SEARCH.add(nameList[i].toUpperCase());
					}
				} else {
					Log.warn("!!! Could not find stop words in the properties files !!!");
				}
			} else {
